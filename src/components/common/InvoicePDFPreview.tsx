  ...
  (
    ...
    const createInvoicePDF = async () => {
      ...
      const pageHeight = pdf.internal.pageSize.getHeight();
          ...
          pdf.setFontSize(22);
          setFont(pdf, 'bold');
          addText('Rechnung', margin, yPos);
          
          pdf.setFontSize(10);
          setFont(pdf, 'normal');
          pdf.setTextColor(...mediumGray);
          addText(`Nr. ${invoiceNumber}`, margin, yPos + 6);
          ...
          setFont(pdf, 'bold');
          pdf.setTextColor(...darkGray);
          addText('RECHNUNGSEMPFÄNGER', margin, yPos);
          ...
          // Invoice Details Boxes with rounded corners (NO BORDERS OR SHADOWS) - MORE PADDING
          ...
          addText('RECHNUNGSDATUM', margin + boxPaddingLeft, yPos + 6); // Increased padding: +5 -> +6
          addText('FÄLLIGKEITSDATUM', margin + boxWidth + 5 + boxPaddingLeft, yPos + 6);
          if (servicePeriodStart && servicePeriodEnd) {
          ...
          setFont(pdf, 'bold');
          pdf.setTextColor(...darkGray);
          addText(formatDate(invoiceDate), margin + boxPaddingLeft, yPos + 13.5); // Increased padding
          addText(formatDate(dueDate), margin + boxWidth + 5 + boxPaddingLeft, yPos + 13.5);
          if (servicePeriodStart && servicePeriodEnd) {
            // Same size as the other dates
            addText(`${formatDate(servicePeriodStart)} - ${formatDate(servicePeriodEnd)}`, margin + 2 * boxWidth + 10 + boxPaddingLeft, yPos + 13.5);
          }

          yPos += boxHeight + 5; // Reduced from 10 to 5
          ...
          items.forEach((item, itemIndex) => {
            ...
            // Unit price
            addText(formatCurrency(item.unitNetPrice), xPos + colWidths[4] - 2, yPos + 6, { align: 'right' });
            xPos += colWidths[4];
          ...
          if (conditions) {
            ...
            setFont(pdf, 'bold');
            pdf.setTextColor(...darkGray);
            addText('ZAHLUNGSBEDINGUNGEN', margin, yPos);
    ...
    useImperativeHandle(ref, () => ({
      ...
      downloadPDF: async () => {
        try {
          ...
          // Download PDF
          pdf.save(`Rechnung_${invoiceNumber}.pdf`);
        } catch (error) {
          console.error('Fehler beim Generieren des PDFs:', error);
          alert('Es gab einen Fehler beim Erstellen des PDFs. Bitte versuchen Sie es erneut.');
        }
